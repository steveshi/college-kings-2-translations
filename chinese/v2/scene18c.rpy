# TODO: Translation updated at 2023-05-10 04:11

# game/v2/scene18c.rpy:25
translate chinese v2s18c_riley_b76aecbf:

    # u "Oh- Hey, Riley. (I gotta start knocking...)"
    u "嘿，莱丽，我要开始敲门了"

# game/v2/scene18c.rpy:30
translate chinese v2s18c_riley_68a2c8f1:

    # ri "Hey, ever heard of knocking?"
    ri "嘿，你不会敲门吗？"

# game/v2/scene18c.rpy:35
translate chinese v2s18c_riley_1e338b87:

    # u "Uh, yeah... sounds familiar. Sorry."
    u "听起来很耳熟，抱歉。"

# game/v2/scene18c.rpy:40
translate chinese v2s18c_riley_8b921c69:

    # ri "Ha, no worries. I was just checking my makeup."
    ri "别担心，我只是在化妆。"

# game/v2/scene18c.rpy:46
translate chinese v2s18c_riley_6cd689cc:

    # ri "So, fifty bucks on your girlfriend's big day, huh? What a sugar daddy..."
    ri "在你女朋友的大日子里赌50块，哈? 真是个甜心老爹..。"

# game/v2/scene18c.rpy:51
translate chinese v2s18c_riley_9b08ec32:

    # u "Yeah, yeah..."
    u "是啊，是啊..。"

# game/v2/scene18c.rpy:56
translate chinese v2s18c_riley_5722ee4b:

    # ri "Next time you need to buy a gift for your girlfriend, just tell me. I can help, you know."
    ri "下次你要给你女朋友买礼物的时候告诉我，我可以帮忙的。"

# game/v2/scene18c.rpy:65
translate chinese v2s18c_riley_08ebfe8e:

    # u "Seriously? That would be great."
    u "真的吗，那太好了。"

# game/v2/scene18c.rpy:71
translate chinese v2s18c_riley_6afe68cb:

    # ri "Yeah. Of course."
    ri "是的，当然。"

# game/v2/scene18c.rpy:77
translate chinese v2s18c_riley_1890e611:

    # ri "Yeah, really. I'd be down whenever."
    ri "是啊，真的，我随时都可以下来。"

# game/v2/scene18c.rpy:82
translate chinese v2s18c_riley_6f21685d:

    # u "What? You can't shop for yourself. That ruins the surprise."
    u "什么? 你不能自己买东西，那会毁了惊喜的。"

# game/v2/scene18c.rpy:88
translate chinese v2s18c_riley_c57b2cb8:

    # ri "You better watch yourself, handsome."
    ri "你最好小心点，帅哥。"

# game/v2/scene18c.rpy:94
translate chinese v2s18c_riley_bdb786d7:

    # ri "Was that supposed to be a pick-up line? Wait, say it again?"
    ri "你是在搭讪吗，等等，再说一遍？"

# game/v2/scene18c.rpy:99
translate chinese v2s18c_riley_f1d56e63:

    # u "*Sighs* Nah, forget it."
    u "* 叹息 * 算了吧。"

# game/v2/scene18c.rpy:104
translate chinese v2s18c_riley_17411807:

    # ri "Besides... after the stunt you pulled tonight, you need all the help you can get."
    ri "再说了，你今晚耍了这么大的花招你需要尽可能多的帮助。"

# game/v2/scene18c.rpy:109
translate chinese v2s18c_riley_f0e698f6:

    # u "Yeah, hopefully she isn't too upset..."
    u "希望她不会太难过。"

# game/v2/scene18c.rpy:114
translate chinese v2s18c_riley_6b8a896c:

    # ri "Oh, please. It'll be just fine, [name]. She's having a good time."
    ri "拜托，没事的,[name]. 她玩得很开心。"

# game/v2/scene18c.rpy:119
translate chinese v2s18c_riley_be3a14ef:

    # u "That's true, she is. She's had a smile on her face all night, apart from when she opened my gift..."
    u "没错，她整晚都面带微笑除了她打开我的礼物时。"

# game/v2/scene18c.rpy:124
translate chinese v2s18c_riley_ecb43893:

    # ri "Stop thinking about it and move on."
    ri "别再想了，向前看。"

# game/v2/scene18c.rpy:129
translate chinese v2s18c_riley_deb1956c:

    # u "Thanks. Good advice, Riley."
    u "谢谢，好建议，莱丽。"

# game/v2/scene18c.rpy:134
translate chinese v2s18c_riley_0b9d93a3:

    # ri "Anytime."
    ri "随时欢迎。"

# game/v2/scene18c.rpy:140
translate chinese v2s18c_riley_b154e2ee:

    # ri "I can't believe you bought the same gift card as Imre..."
    ri "真不敢相信你买了和伊姆雷一样的礼品卡。"

# game/v2/scene18c.rpy:145
translate chinese v2s18c_riley_6c76b8c0:

    # u "Yeah... neither can I, ha."
    u "是啊... 我也不能，哈。"

# game/v2/scene18c.rpy:147
translate chinese v2s18c_riley_33525a70:

    # u "*Sighs* It would have been a lot less embarrassing if she had opened mine first."
    u "如果她先打开我的，就不会这么尴尬了。"

# game/v2/scene18c.rpy:152
translate chinese v2s18c_riley_288486eb:

    # ri "I wouldn't worry about it. Some people are really difficult to shop for."
    ri "我不担心，有些人真的很难找。"

# game/v2/scene18c.rpy:158
translate chinese v2s18c_riley_1b75cc03:

    # ri "Nice job on the gift! Lauren really likes that little horse you bought her."
    ri "礼物做得不错! 劳伦真的很喜欢你给她买的那匹小马。"

# game/v2/scene18c.rpy:163
translate chinese v2s18c_riley_00773ce0:

    # u "Oh, thanks... yeah, shame I didn't buy the right color though, ha."
    u "谢谢，可惜我没买对颜色。"

# game/v2/scene18c.rpy:168
translate chinese v2s18c_riley_2e33d80f:

    # ri "Ah, don't worry about it."
    ri "别担心。"

# game/v2/scene18c.rpy:173
translate chinese v2s18c_riley_861a68a3:

    # ri "It reminds her of the one from her childhood, and that was your goal, right?"
    ri "这让她想起了她童年的那个，那就是你的目标，对吧？"

# game/v2/scene18c.rpy:178
translate chinese v2s18c_riley_2a04f3b2:

    # u "That's true."
    u "那倒是真的。"

# game/v2/scene18c.rpy:183
translate chinese v2s18c_riley_59675b0c:

    # ri "Of course it's true, why would I speak in lies?"
    ri "当然是真的，我为什么要说谎？"

# game/v2/scene18c.rpy:189
translate chinese v2s18c_riley_eb848949:

    # ri "Lauren is obsessed with that damn horse!"
    ri "劳伦对那匹该死的马很着迷！"

# game/v2/scene18c.rpy:195
translate chinese v2s18c_riley_62677e2f:

    # u "Good! Thank God Autumn gave me that insider info."
    u "太好了，谢天谢地奥特姆给了我内部消息。"

# game/v2/scene18c.rpy:200
translate chinese v2s18c_riley_4ebaf5ed:

    # ri "Aw, that was sweet of her."
    ri "她人真好。"

# game/v2/scene18c.rpy:205
translate chinese v2s18c_riley_2b81e731:

    # u "For sure, Autumn is pretty great."
    u "当然，秋天是相当不错的。"

# game/v2/scene18c.rpy:210
translate chinese v2s18c_riley_1f62b0f3:

    # ri "I don't know her that well, really. I should try to hang out with her more."
    ri "我不是很了解她，真的，我应该多和她在一起。"

# game/v2/scene18c.rpy:215
translate chinese v2s18c_riley_49929591:

    # u "Oh boy... you and Autumn in the same room together?"
    u "哦，天啊... 你和奥特姆在同一个房间里？"

# game/v2/scene18c.rpy:220
translate chinese v2s18c_riley_845b2dc2:

    # u "With your combined brainpower, you could take over the world."
    u "凭你的智慧，你可以统治世界。"

# game/v2/scene18c.rpy:225
translate chinese v2s18c_riley_53abfd3c:

    # ri "Take over the world... and fill it with puppies?!"
    ri "占领世界，让世界充满小狗！"

# game/v2/scene18c.rpy:230
translate chinese v2s18c_riley_469533c6:

    # u "That sounds like an extremely healthy ambition. Good luck."
    u "听起来是个非常健康的抱负，祝你好运。"

# game/v2/scene18c.rpy:235
translate chinese v2s18c_riley_332cd2c5:

    # ri "Why thank you, although I don't think we'll need it. Hehe..."
    ri "为什么谢谢你，虽然我不认为我们需要它。呵呵..。"

# game/v2/scene18c.rpy:241
translate chinese v2s18c_riley_ecda0037:

    # ri "You did..."
    ri "你..。"

# game/v2/scene18c.rpy:243
translate chinese v2s18c_riley_16137732:

    # ri "An amazing job with Lauren's gift."
    ri "有劳伦的天赋，你的工作很棒。"

# game/v2/scene18c.rpy:248
translate chinese v2s18c_riley_e4b04c78:

    # u "Oh, phew... thanks."
    u "谢谢。"

# game/v2/scene18c.rpy:253
translate chinese v2s18c_riley_6019a9ce:

    # ri "It was stunning and she's clearly very happy with it."
    ri "很漂亮，她显然对此很满意。"

# game/v2/scene18c.rpy:258
translate chinese v2s18c_riley_9717f5f8:

    # u "I'm happy to hear that; it was pretty great to see how happy she was."
    u "很高兴听你这么说，看到她这么开心真是太好了。"

# game/v2/scene18c.rpy:263
translate chinese v2s18c_riley_159fcf72:

    # u "I love seeing her smile, but being the reason for it is even better, you know?"
    u "我喜欢看她微笑，但是因为这个原因更好，你知道吗？"

# game/v2/scene18c.rpy:274
translate chinese v2s18c_riley_d01618e7:

    # ri "She'll be smiling for a long time after receiving that, haha."
    ri "她收到后会笑很久，哈哈。"

# game/v2/scene18c.rpy:279
translate chinese v2s18c_riley_96321123:

    # u "Ha, yeah, hopefully."
    u "希望如此。"

# game/v2/scene18c.rpy:285
translate chinese v2s18c_riley_687946fd:

    # ri "So, romantic jewelry? *Giggles* For a friend?"
    ri "那么，浪漫的珠宝? 为了一个朋友？"

# game/v2/scene18c.rpy:290
translate chinese v2s18c_riley_018a7d8f:

    # u "I don't-"
    u "我不.."

# game/v2/scene18c.rpy:292
translate chinese v2s18c_riley_6c039ec8:

    # u "I didn't know it would come off as weird, I swear."
    u "我不知道会这么奇怪，我发誓。"

# game/v2/scene18c.rpy:297
translate chinese v2s18c_riley_20f939ed:

    # u "And the sales guy was all \"she'll be a friend for life!\" if I got her jewelry... *Sighs"
    u "那个销售员说\"她会是我一辈子的朋友！\"如果我给她买了首饰... * 叹气"

# game/v2/scene18c.rpy:302
translate chinese v2s18c_riley_7b99fee2:

    # ri "The sales guy?"
    ri "那个销售员？"

# game/v2/scene18c.rpy:307
translate chinese v2s18c_riley_0e18929a:

    # u "Yeah, the-"
    u "是的"

# game/v2/scene18c.rpy:312
translate chinese v2s18c_riley_8c0834a2:

    # ri "You mean the employee whose only goal is to make sure people spend money?"
    ri "你是说那个唯一的目标就是让人们花钱的员工吗？"

# game/v2/scene18c.rpy:317
translate chinese v2s18c_riley_5ef45070:

    # u "Well, when you put it like that..."
    u "既然你这么说了。"

# game/v2/scene18c.rpy:322
translate chinese v2s18c_riley_2143ad6e:

    # u "It sounds like he did his job well."
    u "听起来他工作做得很好。"

# game/v2/scene18c.rpy:327
translate chinese v2s18c_riley_ae0071bf:

    # ri "He definitely did."
    ri "绝对是。"

# game/v2/scene18c.rpy:332
translate chinese v2s18c_riley_55e86340:

    # u "*Sighs* Oh well. She didn't seem to hate it..."
    u "唉，好吧，她似乎并不讨厌。"

# game/v2/scene18c.rpy:337
translate chinese v2s18c_riley_82579852:

    # ri "Riiiight... I wouldn't hate it either. Just a little creepy."
    ri "好吧，我也不会讨厌的，只是有点吓人。"

# game/v2/scene18c.rpy:342
translate chinese v2s18c_riley_e3c27e75:

    # u "(Okay, I'm done being bullied.)"
    u "(好了，我受够被欺负了)"

# game/v2/scene18c.rpy:344
translate chinese v2s18c_riley_d2b1a43b:

    # u "Anyway... I'll leave you to it. Hurry up and get back to the party!"
    u "不管怎样，我就不打扰你们了，赶紧回派对去吧！"

# game/v2/scene18c.rpy:349
translate chinese v2s18c_riley_67f12c35:

    # ri "Okay! See you out there."
    ri "好的，外面见。"

# game/v2/scene18c.rpy:368
translate chinese v2s18c_imre_aubrey_23856dcc:

    # imre "Did you see how happy she was with my gift card?"
    imre "你看到她有多喜欢我的礼品卡了吗？"

# game/v2/scene18c.rpy:373
translate chinese v2s18c_imre_aubrey_20b06fed:

    # au "No, Imre. I didn't."
    au "不，伊姆雷，我没有。"

# game/v2/scene18c.rpy:378
translate chinese v2s18c_imre_aubrey_f488dd90:

    # imre "What do you mean?"
    imre "什么意思？"

# game/v2/scene18c.rpy:383
translate chinese v2s18c_imre_aubrey_aa5d9398:

    # au "It was a gift card, Imre. Why would she be happy about that?"
    au "那是张礼品卡，伊姆雷，她为什么会高兴呢？"

# game/v2/scene18c.rpy:388
translate chinese v2s18c_imre_aubrey_d63b0984:

    # imre "You can buy whatever the fuck you want with it!"
    imre "你可以用它买任何你想要的东西！"

# game/v2/scene18c.rpy:393
translate chinese v2s18c_imre_aubrey_276e9a8a:

    # au "Yeah, I understand the concept. It's just lazy."
    au "是的，我明白这个概念，只是懒惰而已。"

# game/v2/scene18c.rpy:398
translate chinese v2s18c_imre_aubrey_da761984:

    # imre "If someone buys you a gift card in the future, you have to say that to their face."
    imre "如果将来有人给你买礼品卡，你必须当着他们的面说。"

# game/v2/scene18c.rpy:403
translate chinese v2s18c_imre_aubrey_0991ebc7:

    # au "That they're lazy? That it was a boring, sad, and lazy gift?"
    au "说他们懒惰? 说这是个无聊，悲伤，懒惰的礼物？"

# game/v2/scene18c.rpy:408
translate chinese v2s18c_imre_aubrey_83ca5bec:

    # au "Okay. Done."
    au "好的，成交。"

# game/v2/scene18c.rpy:413
translate chinese v2s18c_imre_aubrey_d9d8d264:

    # imre "I know sarcasm when I hear it, Aubrey."
    imre "我一听就知道是讽刺，奥布里。"

# game/v2/scene18c.rpy:416
translate chinese v2s18c_imre_aubrey_465e0e09:

    # imre "Obviously, it wasn't the worst idea since [name] got the exact same one. *Scoffs*"
    imre "显然，这不是最糟糕的主意[name]得到了完全相同的一个。 * 嘲笑 * "

# game/v2/scene18c.rpy:421
translate chinese v2s18c_imre_aubrey_ace0c449:

    # u "Imre, I didn't pick the same gift card as you on purpose. Are you fucking with me?"
    u "伊姆雷，我不是故意跟你选同一张礼品卡的，你在耍我吗？"

# game/v2/scene18c.rpy:426
translate chinese v2s18c_imre_aubrey_615cbd0a:

    # imre "No! I mean... yeah, I was just joking."
    imre "不! 我是说... 是的，我只是在开玩笑。"

# game/v2/scene18c.rpy:431
translate chinese v2s18c_imre_aubrey_580093e4:

    # au "Oh, is that why you've been calling him the gift copier?"
    au "所以你才叫他礼品复印机吗？"

# game/v2/scene18c.rpy:436
translate chinese v2s18c_imre_aubrey_fec7033b:

    # u "Gift copier... good one."
    u "礼品复印机... 不错。"

# game/v2/scene18c.rpy:441
translate chinese v2s18c_imre_aubrey_691143b7:

    # imre "*Sighs* All right, chill. I'm over it."
    imre "好了，冷静，我受够了。"

# game/v2/scene18c.rpy:446
translate chinese v2s18c_imre_aubrey_9c35509c:

    # au "I would hope so..."
    au "希望如此。"

# game/v2/scene18c.rpy:451
translate chinese v2s18c_imre_aubrey_52aae21f:

    # imre "Why are you being such a bitch tonight?"
    imre "你今晚为什么这么贱？"

# game/v2/scene18c.rpy:456
translate chinese v2s18c_imre_aubrey_e95bdb1c:

    # u "Whoa-"
    u "哇"

# game/v2/scene18c.rpy:461
translate chinese v2s18c_imre_aubrey_3adb6801:

    # imre "Ohhh, okay... she gets to bully me about buying a gift card and I don't get to call her a bitch?"
    imre "哦，好吧... 她可以在买礼品卡的事上欺负我而我却不能叫她贱人？"

# game/v2/scene18c.rpy:466
translate chinese v2s18c_imre_aubrey_53ccb321:

    # u "That's correct. Yeah."
    u "没错。"

# game/v2/scene18c.rpy:471
translate chinese v2s18c_imre_aubrey_c3d109f3:

    # au "*Giggles*"
    au "* 咯咯笑 *"

# game/v2/scene18c.rpy:476
translate chinese v2s18c_imre_aubrey_bdab05f3:

    # imre "Fuck this, man... I know Lauren's smiling right now because of me."
    imre "去他妈的，我知道劳伦现在笑是因为我。"

# game/v2/scene18c.rpy:482
translate chinese v2s18c_imre_aubrey_dccfc27a:

    # au "A good gift is whatever the fuck [name] got. It seemed to bring back some fun memories."
    au "一个好的礼物是什么他妈的[name]这似乎勾起了一些有趣的回忆。"

# game/v2/scene18c.rpy:487
translate chinese v2s18c_imre_aubrey_952a5c83:

    # imre "Pfft... a toy?"
    imre "玩具？"

# game/v2/scene18c.rpy:493
translate chinese v2s18c_imre_aubrey_5e925fcd:

    # u "Oh yeah, Autumn gave me the idea. I'm not sure what I would've gotten Lauren without her help."
    u "是的，奥特姆给了我这个主意。我不知道如果没有劳伦的帮助我会得到什么。"

# game/v2/scene18c.rpy:498
translate chinese v2s18c_imre_aubrey_628f685f:

    # au "Hopefully not a gift card..."
    au "希望不是礼品卡。"

# game/v2/scene18c.rpy:503
translate chinese v2s18c_imre_aubrey_90422ad0:

    # u "Oh, God no... of course not..."
    u "哦，上帝，不... 当然不是..。"

# game/v2/scene18c.rpy:508
translate chinese v2s18c_imre_aubrey_3666e431:

    # imre "Okay-"
    imre "好吧"

# game/v2/scene18c.rpy:514
translate chinese v2s18c_imre_aubrey_b8f21112:

    # au "An actual good gift was the jewelry that [name] got."
    au "真正好的礼物是[name]拿到了。"

# game/v2/scene18c.rpy:519
translate chinese v2s18c_imre_aubrey_1ce3d42a:

    # u "Ha, thanks. Her reaction couldn't have been better."
    u "谢谢，她的反应好得不能再好了。"

# game/v2/scene18c.rpy:524
translate chinese v2s18c_imre_aubrey_c1648138:

    # imre "Yeah, but still... with my gift card, she can go out and buy something even better than that."
    imre "是的，但是... 有了我的礼品卡，她可以出去买更好的东西。"

# game/v2/scene18c.rpy:529
translate chinese v2s18c_imre_aubrey_47d06f92:

    # au "Imre, drop it. Your gift card was trash."
    au "伊姆雷，别说了，你的礼品卡是垃圾。"

# game/v2/scene18c.rpy:534
translate chinese v2s18c_imre_aubrey_2a07f85e:

    # u "What's better than jewelry?"
    u "有什么比珠宝更好呢？"

# game/v2/scene18c.rpy:539
translate chinese v2s18c_imre_aubrey_722f11d9:

    # imre "Umm, a lot of things?"
    imre "很多事吗？"

# game/v2/scene18c.rpy:544
translate chinese v2s18c_imre_aubrey_195b323b:

    # au "Go ahead, Imre. Name one. Name one thing that's better than jewelry."
    au "说吧，伊姆雷，说一个，说一个比珠宝更好的东西。"

# game/v2/scene18c.rpy:549
translate chinese v2s18c_imre_aubrey_f930a6ce:

    # imre "You know what? I don't need this."
    imre "你知道吗? 我不需要这个。"

# game/v2/scene18c.rpy:555
translate chinese v2s18c_imre_aubrey_fdbddd5b:

    # imre "At least I didn't buy her a romantic piece of jewelry! That was so fucking awkward, dude."
    imre "- 至少我没给她买浪漫的首饰!-太他妈尴尬了，伙计。"

# game/v2/scene18c.rpy:560
translate chinese v2s18c_imre_aubrey_6acaec58:

    # u "Seriously, guys... I didn't know it'd be weird."
    u "说真的，伙计们，我不知道会这么奇怪。"

# game/v2/scene18c.rpy:565
translate chinese v2s18c_imre_aubrey_2b197437:

    # au "I think getting creepy jewelry is much better than getting a plain piece of plastic with money on it..."
    au "我认为得到令人毛骨悚然的首饰比得到一块普通的塑料上面有钱要好得多..。"

# game/v2/scene18c.rpy:567
translate chinese v2s18c_imre_aubrey_49d00f52:

    # u "(Creepy jewelry?)"
    u "(令人毛骨悚然的珠宝?)"

# game/v2/scene18c.rpy:572
translate chinese v2s18c_imre_aubrey_30a6eb31:

    # imre "Aubrey, enough is enough! My gift card is the reason that Lauren is so happy right now. Admit it."
    imre "奥布里，够了，我的礼品卡就是劳伦现在这么开心的原因，承认吧。"

# game/v2/scene18c.rpy:577
translate chinese v2s18c_imre_aubrey_a51aaa7f:

    # au "Ha!"
    au "哈！"

# game/v2/scene18c.rpy:579
translate chinese v2s18c_imre_aubrey_d192d299:

    # au "Okay, keep telling yourself that."
    au "好吧，继续自欺欺人吧。"

# game/v2/scene18c.rpy:584
translate chinese v2s18c_imre_aubrey_bca76fc4:

    # imre "I will."
    imre "我会的。"

# game/v2/scene18c.rpy:594
translate chinese v2s18c_imre_aubrey_3b33a619:

    # au "Nice job! I've been trying to scare him away for years..."
    au "干得漂亮! 这么多年来我一直想把他吓跑..。"

# game/v2/scene18c.rpy:599
translate chinese v2s18c_imre_aubrey_13b9271d:

    # u "Don't blame that on me! You're the one who got him fired up."
    u "别怪我，是你让他发火的。"

# game/v2/scene18c.rpy:604
translate chinese v2s18c_imre_aubrey_b7f3e4bc:

    # au "Who cares? It's too easy."
    au "谁在乎啊，太简单了。"

# game/v2/scene18c.rpy:609
translate chinese v2s18c_imre_aubrey_f93a8ae8:

    # u "Very true."
    u "没错。"

# game/v2/scene18c.rpy:612
translate chinese v2s18c_imre_aubrey_6e2a344e:

    # u "(Now is a good time to check something off the list, should I ask Aubrey for help?)"
    u "(现在是时候从清单上划掉一些东西了，我应该向奥布里寻求帮助吗?)"

# game/v2/scene18c.rpy:619
translate chinese v2s18c_imre_aubrey_3913d396:

    # u "So, I'm just curious..."
    u "我只是好奇。"

# game/v2/scene18c.rpy:624
translate chinese v2s18c_imre_aubrey_5b3016dd:

    # au "Yeah?"
    au "是吗？"

# game/v2/scene18c.rpy:629
translate chinese v2s18c_imre_aubrey_75b0c30a:

    # u "If I told you I had this dumb list of tasks to complete by the end of the night, would you want to help me?"
    u "如果我告诉你，我今晚要完成一大堆任务你愿意帮我吗？"

# game/v2/scene18c.rpy:634
translate chinese v2s18c_imre_aubrey_c47e6fce:

    # au "Hmm... What kind of tasks?"
    au "什么样的任务？"

# game/v2/scene18c.rpy:639
translate chinese v2s18c_imre_aubrey_b1d790bb:

    # u "Uh, well..."
    u "这个嘛。"

# game/v2/scene18c.rpy:649
translate chinese v2s18c_imre_aubrey_71909011:

    # au "Oh! Ha..."
    au "哦! 哈..。"

# game/v2/scene18c.rpy:654
translate chinese v2s18c_imre_aubrey_94e68ad3:

    # u "Yeah... I just thought I'd ask because-"
    u "是的... 我只是想问问，因为.."

# game/v2/scene18c.rpy:659
translate chinese v2s18c_imre_aubrey_6fd6e03b:

    # u "(Not feeling it...)"
    u "(没有感觉...)"

# game/v2/scene18c.rpy:664
translate chinese v2s18c_imre_aubrey_ba1f545d:

    # au "This might be shocking, but... I'm kind of ready to leave."
    au "这可能有点吓人，但是... 我已经准备好离开了。"

# game/v2/scene18c.rpy:669
translate chinese v2s18c_imre_aubrey_4810a42b:

    # u "Wait, really? Since when do you get tired before midnight?"
    u "等等，真的吗，你什么时候开始午夜前就累了？"

# game/v2/scene18c.rpy:674
translate chinese v2s18c_imre_aubrey_9fe4cbf6:

    # au "Right? I guess it's just been a lot lately, modelling and the wedding and all that."
    au "对吧，我想最近一定是太忙了模特啊，婚礼啊什么的。"

# game/v2/scene18c.rpy:679
translate chinese v2s18c_imre_aubrey_2d9003e5:

    # u "Yeah. makes sense."
    u "是啊，有道理。"

# game/v2/scene18c.rpy:681
translate chinese v2s18c_imre_aubrey_ec0421c1:

    # u "You definitely need to get a good rest or else you'll be falling asleep during the vows."
    u "你绝对需要好好休息否则你会在宣誓的时候睡着的。"

# game/v2/scene18c.rpy:686
translate chinese v2s18c_imre_aubrey_d766f963:

    # au "Yeah, I'm going to."
    au "是的，我会的。"

# game/v2/scene18c.rpy:691
translate chinese v2s18c_imre_aubrey_00423e97:

    # u "Good..."
    u "好..。"

# game/v2/scene18c.rpy:693
translate chinese v2s18c_imre_aubrey_5e1ab662:

    # u "I'm going to talk to the others before it's time to go, find me if you need me, yeah?"
    u "我要在离开前和其他人谈谈，如果你需要我就来找我，好吗？"

# game/v2/scene18c.rpy:698
translate chinese v2s18c_imre_aubrey_486f8c31:

    # au "Okay. Will do."
    au "好的，我会的。"

# game/v2/scene18c.rpy:716
translate chinese v2s18c_imre_aubrey_802b1e3d:

    # au "So... do you still need some help with that list you mentioned earlier?"
    au "那么... 你之前提到的名单还需要帮忙吗？"

# game/v2/scene18c.rpy:721
translate chinese v2s18c_imre_aubrey_1b3da86d:

    # u "Yeah, if you want-"
    u "是啊，如果你想.."

# game/v2/scene18c.rpy:727
translate chinese v2s18c_aubreysg_bea2f980:

    # au "Come on then..."
    au "来吧。"

# game/v2/scene18c.rpy:740
translate chinese v2s18c_aubreysg_3b84e907:

    # au "*Whispers* Nice! There's nobody in here..."
    au "很好，这里没人。"

# game/v2/scene18c.rpy:745
translate chinese v2s18c_aubreysg_eff3b904:

    # u "Autumn's room?"
    u "奥顿的房间？"

# game/v2/scene18c.rpy:750
translate chinese v2s18c_aubreysg_6715cb8a:

    # au "Yeah, so? What she doesn't know won't hurt her."
    au "是啊，那又怎样? 她不知道又不会伤害到她。"

# game/v2/scene18c.rpy:755
translate chinese v2s18c_aubreysg_92983e5c:

    # u "(Well, when you put it like that...)"
    u "(你这么一说...)"

# game/v2/scene18c.rpy:768
translate chinese v2s18c_aubreysg_5482ff6d:

    # au "Okay..."
    au "好吧..。"

# game/v2/scene18c.rpy:773
translate chinese v2s18c_aubreysg_c850bb96:

    # au "I'm all yours for pleasing."
    au "取悦你是我的荣幸。"

# game/v2/scene18c.rpy:780
translate chinese v2s18c_aubreysg_619fab1b:

    # u "(Ha... damn...)"
    u "(哈... 该死...)"

# game/v2/scene18c.rpy:811
translate chinese v2s18c_aubreysg_ac4ef2cf:

    # au "*Moans* Touch me..."
    au "* 呻吟 * 触摸我。"

# game/v2/scene18c.rpy:818
translate chinese v2s18c_aubreysg_e338cb02:

    # au "*Gasps* [name]..."
    au "* 喘息 *[name]..."

# game/v2/scene18c.rpy:825
translate chinese v2s18c_aubreysg_1f049864:

    # au "Please."
    au "求你了。"

# game/v2/scene18c.rpy:830
translate chinese v2s18c_aubreysg_0e94215b:

    # u "How about a finger?"
    u "一根手指怎么样？"

# game/v2/scene18c.rpy:847
translate chinese v2s18c_aubreysg_11e71c0b:

    # au "Oh... shiiit... *Moans*"
    au "哦... 该死... * 呻吟 * "

# game/v2/scene18c.rpy:849
translate chinese v2s18c_aubreysg_9aadcc33:

    # u "How does that feel?"
    u "感觉怎么样？"

# game/v2/scene18c.rpy:851
translate chinese v2s18c_aubreysg_125f9078:

    # au "Ha... *Panting* it feels amazing."
    au "哈... * 喘气 * 感觉太棒了。"

# game/v2/scene18c.rpy:856
translate chinese v2s18c_aubreysg_e694a7ac:

    # au "*Moans* Yes... [name], please..."
    au "* 呻吟 * 是的..。[name]求你了。"

# game/v2/scene18c.rpy:858
translate chinese v2s18c_aubreysg_68945b2a:

    # au "Mmm, fuck!"
    au "妈的！"

# game/v2/scene18c.rpy:860
translate chinese v2s18c_aubreysg_11759ca6:

    # au "*Moans* YES!"
    au "* 呻吟 * 是的！"

# game/v2/scene18c.rpy:873
translate chinese v2s18c_aubreysg_0c85aa2e:

    # u "Shh, shh, shh..."
    u "嘘，嘘，嘘..。"

# game/v2/scene18c.rpy:875
translate chinese v2s18c_aubreysg_445b5f59:

    # u "We don't want anyone interrupting us, do we?"
    u "我们不希望有人打扰我们，对吧？"

# game/v2/scene18c.rpy:880
translate chinese v2s18c_aubreysg_548bb5fa:

    # au "*Moans* N-no."
    au "* 呻吟 * 不。"

# game/v2/scene18c.rpy:886
translate chinese v2s18c_aubreysg_8f9dd420:

    # u "Scream for me."
    u "为我尖叫。"

# game/v2/scene18c.rpy:888
translate chinese v2s18c_aubreysg_6f3f6dcd:

    # au "*Moans* You-"
    au "* 呻吟 * 你"

# game/v2/scene18c.rpy:890
translate chinese v2s18c_aubreysg_c1ce36ef:

    # au "OH!"
    au "哦！"

# game/v2/scene18c.rpy:895
translate chinese v2s18c_aubreysg_6f372b8e:

    # au "Fuck... you don't want that... *Moans*"
    au "操... 你不会想要那个的... * 呻吟 * "

# game/v2/scene18c.rpy:897
translate chinese v2s18c_aubreysg_174d4484:

    # u "(Maybe. But I definitely don't want anyone else to hear her. Hmm...)"
    u "(也许吧，但我绝对不想让别人听到她的声音，嗯...)"

# game/v2/scene18c.rpy:904
translate chinese v2s18c_aubreysg_a4e2e281:

    # au "Mmm!"
    au "嗯！"

# game/v2/scene18c.rpy:909
translate chinese v2s18c_aubreysg_a6b74049:

    # u "Faster?"
    u "更快？"

# game/v2/scene18c.rpy:916
translate chinese v2s18c_aubreysg_3da13d1c:

    # au "*Moans*"
    au "* 呻吟 *"

# game/v2/scene18c.rpy:921
translate chinese v2s18c_aubreysg_4b576d42:

    # u "You're so wet..."
    u "你湿透了。"

# game/v2/scene18c.rpy:923
translate chinese v2s18c_aubreysg_6efd31ee:

    # au "Mmm, mmm! *Moans*"
    au "* 呻吟 * "

# game/v2/scene18c.rpy:929
translate chinese v2s18c_aubreysg_7063425a:

    # au "I'm-"
    au "我.."

# game/v2/scene18c.rpy:934
translate chinese v2s18c_aubreysg_7c3e62f2:

    # au "Ffffuck... [name]."
    au "去你妈的。[name]."

# game/v2/scene18c.rpy:936
translate chinese v2s18c_aubreysg_bb2f60d1:

    # u "Are you gonna cum for me?"
    u "你要为我高潮吗？"

# game/v2/scene18c.rpy:943
translate chinese v2s18c_aubreysg_36b40086:

    # au "*Whispers* Yes..."
    au "* 低语 * 是的。"

# game/v2/scene18c.rpy:945
translate chinese v2s18c_aubreysg_2243895b:

    # u "Now?"
    u "现在吗？"

# game/v2/scene18c.rpy:952
translate chinese v2s18c_aubreysg_7120c117:

    # au "Mmm, yes... *Moans* I-"
    au "嗯，是的... * 呻吟 * 我.."

# game/v2/scene18c.rpy:957
translate chinese v2s18c_aubreysg_9867192c:

    # au "Fuck... Oh, [name]..."
    au "操... 哦,[name]..."

# game/v2/scene18c.rpy:959
translate chinese v2s18c_aubreysg_1338134a:

    # au "*Moans* I'm gonna cum."
    au "* 呻吟 * 我要射了。"

# game/v2/scene18c.rpy:967
translate chinese v2s18c_aubreysg_6bcd435f:

    # u "Yeah?"
    u "是吗？"

# game/v2/scene18c.rpy:974
translate chinese v2s18c_aubreysg_98d091cc:

    # au "Yessss... Yes, yes!"
    au "是的... 是的，是的！"

# game/v2/scene18c.rpy:979
translate chinese v2s18c_aubreysg_30a90cc9:

    # u "(And... check.)"
    u "好了"

# game/v2/scene18c.rpy:984
translate chinese v2s18c_aubreysg_40a168ac:

    # au "*Groans*"
    au "* 呻吟 *"

# game/v2/scene18c.rpy:992
translate chinese v2s18c_nsfwSkipLabelAub_f979f308:

    # au "*Panting* Holy shit, [name]."
    au "* 气喘吁吁 * 我靠,[name]."

# game/v2/scene18c.rpy:997
translate chinese v2s18c_nsfwSkipLabelAub_ad586092:

    # u "Looked like you enjoyed that."
    u "看起来你很享受啊。"

# game/v2/scene18c.rpy:1002
translate chinese v2s18c_nsfwSkipLabelAub_a8f2ee86:

    # au "Yes... *Panting*"
    au "是的... * 气喘吁吁 * "

# game/v2/scene18c.rpy:1014
translate chinese v2s18c_nsfwSkipLabelAub_35a07e5a:

    # au "Mmmm..."
    au "嗯..。"

# game/v2/scene18c.rpy:1019
translate chinese v2s18c_nsfwSkipLabelAub_24abcc3a:

    # au "I really, really, really did."
    au "我真的，真的，真的。"

# game/v2/scene18c.rpy:1026
translate chinese v2s18c_nsfwSkipLabelAub_e3e15647:

    # u "Well, I'm glad. And thank you."
    u "我很高兴，谢谢你。"

# game/v2/scene18c.rpy:1033
translate chinese v2s18c_nsfwSkipLabelAub_660e8ec7:

    # au "Are you kidding?"
    au "你在开玩笑吗？"

# game/v2/scene18c.rpy:1038
translate chinese v2s18c_nsfwSkipLabelAub_ef2f1671:

    # u "What?"
    u "什么？"

# game/v2/scene18c.rpy:1043
translate chinese v2s18c_nsfwSkipLabelAub_a8bfda7b:

    # au "Thank me? Thank you."
    au "谢我? 谢谢。"

# game/v2/scene18c.rpy:1062
translate chinese v2s18c_nsfwSkipLabelAub_5b494f57:

    # u "*Gulps* You're welcome."
    u "* 咕噜咕噜 * 不客气。"

# game/v2/scene18c.rpy:1069
translate chinese v2s18c_nsfwSkipLabelAub_487dba53:

    # au "We better get back to the party now, heh."
    au "我们最好现在就回派对去。"

# game/v2/scene18c.rpy:1074
translate chinese v2s18c_nsfwSkipLabelAub_02a43133:

    # u "Yeah, good idea."
    u "好主意。"

# game/v2/scene18c.rpy:1080
translate chinese v2s18c_nsfwSkipLabelAub_121fba87:

    # au "I'll miss you."
    au "我会想你的。"

# game/v2/scene18c.rpy:1097
translate chinese v2s18c_nsfwSkipLabelAub_f5e70424:

    # u "*Exhales* (Damn, what a night so far.)"
    u "* 呼气 * (该死，到目前为止真是个美妙的夜晚)"

# game/v2/scene18c.rpy:1107
translate chinese v2s18c_nsfwSkipLabelAub_84d83784:

    # u "(Now would be the perfect chance to steal a pair of panties for the list... I just hope Autumn won't notice.)"
    u "(现在是偷一条内裤上榜的绝佳机会... ... 我只希望奥特姆不会注意到。)"

# game/v2/scene18c.rpy:1114
translate chinese v2s18c_nsfwSkipLabelAub_efc25783:

    # u "(Sorry, Autumn. I'm a man in need.)"
    u "(对不起，奥顿，我需要帮助)"

# game/v2/scene18c.rpy:1122
translate chinese v2s18c_nsfwSkipLabelAub_318385ef:

    # u "(Double check.)"
    u "(再确认一次)"

# game/v2/scene18c.rpy:1128
translate chinese v2s18c_nsfwSkipLabelAub_8e01be0b:

    # u "(There's no way in hell I'm taking my girlfriend's sister's underwear. Not happening.)"
    u "(我绝对不可能拿走我女朋友姐姐的内裤，绝对不行。)"

# game/v2/scene18c.rpy:1130
translate chinese v2s18c_nsfwSkipLabelAub_f140851f:

    # u "(There's no way I'm taking those. Bad idea. Very, very bad idea.)"
    u "(我才不要吃这些，坏主意，非常非常坏的主意)"

# game/v2/scene18c.rpy:1154
translate chinese v2s18c_ryan_adddb164:

    # u "Ryan? Are you sleeping?"
    u "Ryan? 你睡着了吗？"

# game/v2/scene18c.rpy:1159
translate chinese v2s18c_ryan_ff2376e3:

    # ry "*Groans*"
    ry "* 呻吟 *"

# game/v2/scene18c.rpy:1164
translate chinese v2s18c_ryan_b8a616dc:

    # u "Damn, dude... still not feeling well?"
    u "该死，老兄，还是不舒服吗？"

# game/v2/scene18c.rpy:1169
translate chinese v2s18c_ryan_44ba0674:

    # ry "*Whining*"
    ry "* 抱怨 * "

# game/v2/scene18c.rpy:1174
translate chinese v2s18c_ryan_6a6149a4:

    # u "(For fuck's sake...)"
    u "(看在他妈的份上...)"

# game/v2/scene18c.rpy:1179
translate chinese v2s18c_ryan_533a4c06:

    # ry "I've gotten sick so many times I lost count."
    ry "我病了那么多次，都数不清了。"

# game/v2/scene18c.rpy:1184
translate chinese v2s18c_ryan_8e3c3926:

    # u "Ugh, I'm sorry man. I wish I could do something to help."
    u "对不起，伙计，我希望我能帮上忙。"

# game/v2/scene18c.rpy:1189
translate chinese v2s18c_ryan_ec5a8b60:

    # ry "Keep me away from candy. Forever..."
    ry "让我永远远离糖果。"

# game/v2/scene18c.rpy:1194
translate chinese v2s18c_ryan_a657c661:

    # u "Ha, all right. Noted."
    u "好吧，知道了。"

# game/v2/scene18c.rpy:1199
translate chinese v2s18c_ryan_34cf457d:

    # ry "No... *sniffles* there's nothing anyone can do."
    ry "不... * 流鼻涕 * 没人能做什么。"

# game/v2/scene18c.rpy:1204
translate chinese v2s18c_ryan_1067b076:

    # u "(Is he... crying?)"
    u "(他在... 哭吗?)"

# game/v2/scene18c.rpy:1209
translate chinese v2s18c_ryan_ee91fea3:

    # ry "I'm gonna sleep now."
    ry "我要睡了。"

# game/v2/scene18c.rpy:1214
translate chinese v2s18c_ryan_8296abcc:

    # u "All right, are you sure you're okay?"
    u "你确定你没事吗？"

# game/v2/scene18c.rpy:1219
translate chinese v2s18c_ryan_a197efe0:

    # ry "..."
    ry "..."

# game/v2/scene18c.rpy:1224
translate chinese v2s18c_ryan_f0ff2a68:

    # u "Ryan?"
    u "莱恩？"

# game/v2/scene18c.rpy:1229
translate chinese v2s18c_ryan_98cdb679:

    # ry "*Snoring*"
    ry "* 打呼噜 * "

# game/v2/scene18c.rpy:1234
translate chinese v2s18c_ryan_57ae9944:

    # u "*Sighs*"
    u "* 叹息 *"

# game/v2/scene18c.rpy:1239
translate chinese v2s18c_ryan_bb4c1ef2:

    # u "(Oh, wait! I need that condom if I want to complete the list...)"
    u "(哦，等等! 如果我想完成清单，我需要那个避孕套...)"

# game/v2/scene18c.rpy:1248
translate chinese v2s18c_ryan_5c28fcdd:

    # u "(Sorry, bro...)"
    u "(Sorry, bro...)"

# game/v2/scene18c.rpy:1253
translate chinese v2s18c_ryan_ff2376e3_1:

    # ry "*Groans*"
    ry "* 呻吟 *"

# game/v2/scene18c.rpy:1255
translate chinese v2s18c_ryan_0224ce63:

    # u "(Fuck!)"
    u "(操!)"

# game/v2/scene18c.rpy:1257
translate chinese v2s18c_ryan_a197efe0_1:

    # ry "..."
    ry "..."

# game/v2/scene18c.rpy:1262
translate chinese v2s18c_ryan_e79bb6d7:

    # u "Phew..."
    u "呼..。"

# game/v2/scene18c.rpy:1267
translate chinese v2s18c_ryan_64461017:

    # u "(This thing is definitely expired.)"
    u "(这玩意肯定过期了)"

# game/v2/scene18c.rpy:1274
translate chinese v2s18c_ryan_1a25e95b:

    # u "*Whispers* Hey, thanks man!"
    u "嘿，谢谢你，伙计！"

# game/v2/scene18c.rpy:1282
translate chinese v2s18c_ryan_49e9ad5e:

    # u "(Ah, I can't do that to him...)"
    u "(啊，我不能这样对他...)"

# game/v2/scene18c.rpy:1284
translate chinese v2s18c_ryan_f726dd1b:

    # u "(He's probably had that thing for a few years; it'd be like stealing his best friend.)"
    u "(他可能已经这样好几年了，这就像偷走他最好的朋友一样。)"

# game/v2/scene18c.rpy:1289
translate chinese v2s18c_ryan_e58ad414:

    # u "Good luck, Ryan."
    u "祝你好运 Ryan。"

# game/v2/scene18c.rpy:1316
translate chinese v2s18c_lauren_e2d57163:

    # u "Hey, there you are. Hiding from your own party?"
    u "你在这儿啊，躲自己的派对吗？"

# game/v2/scene18c.rpy:1321
translate chinese v2s18c_lauren_75448f59:

    # la "Oh, hey."
    la "哦，嘿。"

# game/v2/scene18c.rpy:1323
translate chinese v2s18c_lauren_3d625104:

    # la "No, I'm having a great time..."
    la "不，我玩得很开心。"

# game/v2/scene18c.rpy:1325
translate chinese v2s18c_lauren_d828f987:

    # la "Just taking a moment to soak it all in, you know?"
    la "花点时间好好享受一下？"

# game/v2/scene18c.rpy:1330
translate chinese v2s18c_lauren_65ccfb9d:

    # u "Yeah, I get it."
    u "是的，我明白。"

# game/v2/scene18c.rpy:1340
translate chinese v2s18c_lauren_82b8fa01:

    # u "It's so nice up here."
    u "这里真不错。"

# game/v2/scene18c.rpy:1345
translate chinese v2s18c_lauren_630bf3d2:

    # la "Yeah, it is. Autumn's let me stay in this room for tonight."
    la "是的，奥顿让我今晚住这个房间。"

# game/v2/scene18c.rpy:1347
translate chinese v2s18c_lauren_7806df45:

    # la "Her room is even bigger than this one. Have you seen it?"
    la "她的房间比这个还大，你见过吗？"

# game/v2/scene18c.rpy:1356
translate chinese v2s18c_lauren_dcbdc171:

    # u "Eh, I like the balcony a lot better. I'd be out here all the time if I were you."
    u "我更喜欢阳台，如果我是你，我会一直待在这里。"

# game/v2/scene18c.rpy:1361
translate chinese v2s18c_lauren_7ec1e741:

    # la "I drop by here a lot. It's relaxing."
    la "我经常来这里，很放松。"

# game/v2/scene18c.rpy:1366
translate chinese v2s18c_lauren_ff8b2c51:

    # u "Yeah... having a small dorm room kind of sucks. The bigger the better, in my opinion."
    u "是啊... 有一个小的宿舍有点糟糕。越大越好，在我看来。"

# game/v2/scene18c.rpy:1371
translate chinese v2s18c_lauren_a1159189:

    # la "I mean, it does suck sometimes, but I don't need something massive."
    la "我是说，有时候确实很糟糕，但我不需要那么大的东西。"

# game/v2/scene18c.rpy:1376
translate chinese v2s18c_lauren_662b6819:

    # u "To each their own, ha."
    u "人各有志，哈。"

# game/v2/scene18c.rpy:1381
translate chinese v2s18c_lauren_792943a0:

    # la "I can't believe how stressed I was about this party... I thought everything was going wrong, but... no."
    la "我真不敢相信这个派对让我压力这么大，我以为一切都出问题了，但是... 没有。"

# game/v2/scene18c.rpy:1386
translate chinese v2s18c_lauren_a0135ab4:

    # la "It's all been perfect so far and everyone showed up!"
    la "到目前为止一切都很完美，大家都来了！"

# game/v2/scene18c.rpy:1391
translate chinese v2s18c_lauren_dbec4a25:

    # u "Of course everyone showed up! You're a pretty popular girl, Lauren."
    u "当然每个人都来了! 你是个很受欢迎的女孩，劳伦。"

# game/v2/scene18c.rpy:1396
translate chinese v2s18c_lauren_e364c794:

    # la "Ha... I guess I just didn't see myself that way."
    la "哈... 我想我只是没有那样看待自己。"

# game/v2/scene18c.rpy:1401
translate chinese v2s18c_lauren_a8722e91:

    # la "I'm so grateful for all of our friends."
    la "我很感激我们所有的朋友。"

# game/v2/scene18c.rpy:1406
translate chinese v2s18c_lauren_c508878c:

    # u "Me too, really. Everyone seems to be having a great time, too."
    u "我也是，真的，大家似乎都玩得很开心。"

# game/v2/scene18c.rpy:1412
translate chinese v2s18c_lauren_a55420cc:

    # u "Well, everyone besides Ryan..."
    u "除了瑞恩以外的所有人。"

# game/v2/scene18c.rpy:1418
translate chinese v2s18c_lauren_f6973ae5:

    # la "Ha! I know. Well... *sighs* I shouldn't be laughing..."
    la "哈! 我知道，好吧... * 叹息 * 我不该笑的..。"

# game/v2/scene18c.rpy:1420
translate chinese v2s18c_lauren_3f19e68b:

    # la "Besides Ryan! He's been getting sick all night. Something about eating too much. *Giggles*"
    la "除了莱恩! 他整晚都在生病。好像是吃得太多了。 * 咯咯笑 * "

# game/v2/scene18c.rpy:1425
translate chinese v2s18c_lauren_bf91f827:

    # u "Did he arrive like that? What even happened to him?"
    u "他就这么来了? 他到底怎么了？"

# game/v2/scene18c.rpy:1430
translate chinese v2s18c_lauren_b1300774:

    # la "No idea."
    la "不知道。"

# game/v2/scene18c.rpy:1435
translate chinese v2s18c_lauren_7879ec25:

    # la "He looked okay when he first got here, but I do remember him holding his stomach a lot."
    la "他刚来的时候还好好的，但我记得他经常捂着肚子。"

# game/v2/scene18c.rpy:1437
translate chinese v2s18c_lauren_4b52db76:

    # la "Then he just disappeared into the bathroom for ages."
    la "然后他就在浴室里消失了很久。"

# game/v2/scene18c.rpy:1442
translate chinese v2s18c_lauren_1ed2610d:

    # u "Jeez... poor Ryan."
    u "天啊... 可怜的 Ryan。"

# game/v2/scene18c.rpy:1447
translate chinese v2s18c_lauren_4a69a823:

    # la "Hopefully he can recover enough to walk home."
    la "希望他能恢复到能走路回家。"

# game/v2/scene18c.rpy:1452
translate chinese v2s18c_lauren_a68a7da7:

    # u "Oof."
    u "哦。"

# game/v2/scene18c.rpy:1457
translate chinese v2s18c_lauren_9ba0d572:

    # la "I mean, uh... hopefully he feels better soon..."
    la "我是说，希望他能尽快好起来。"

# game/v2/scene18c.rpy:1462
translate chinese v2s18c_lauren_9945103a:

    # u "Yeah, I hope so too."
    u "我也希望如此。"

# game/v2/scene18c.rpy:1465
translate chinese v2s18c_lauren_8d33cc11:

    # u "By the way, you look amazing tonight."
    u "顺便说一句，你今晚看起来棒极了。"

# game/v2/scene18c.rpy:1470
translate chinese v2s18c_lauren_ef1bb426:

    # la "Aww! Come here, you."
    la "过来。"

# game/v2/scene18c.rpy:1482
translate chinese v2s18c_lauren_c631639e:

    # la "Thank you, [name]."
    la "谢谢,[name]."

# game/v2/scene18c.rpy:1487
translate chinese v2s18c_lauren_7ccc0fdc:

    # u "You're very welcome."
    u "不客气。"

# game/v2/scene18c.rpy:1497
translate chinese v2s18c_lauren_4eebb931:

    # la "I don't know if I've ever felt happier..."
    la "我不知道我是否曾经感到快乐..。"

# game/v2/scene18c.rpy:1502
translate chinese v2s18c_lauren_d42b418c:

    # u "Me either."
    u "我也是。"

# game/v2/scene18c.rpy:1507
translate chinese v2s18c_lauren_43f96172:

    # la "Well, speaking of Ryan..."
    la "说到 Ryan..。"

# game/v2/scene18c.rpy:1509
translate chinese v2s18c_lauren_3d97cac0:

    # la "*Sighs* I should take him some water and see how he's doing."
    la "我该给他倒点水，看看他怎么样了。"

# game/v2/scene18c.rpy:1514
translate chinese v2s18c_lauren_d475986f:

    # u "Maybe. Or maybe you're always looking out for other people, and not enough for yourself..."
    u "也许吧，也许你总是关心别人，却不够关心自己。"

# game/v2/scene18c.rpy:1519
translate chinese v2s18c_lauren_adbc94ec:

    # la "Hehe, that's what you're here for."
    la "呵呵，这就是你来的原因。"

# game/v2/scene18c.rpy:1524
translate chinese v2s18c_lauren_fbd6a8e2:

    # u "(Cheeky...)"
    u "(厚脸皮...)"

# game/v2/scene18c.rpy:1541
translate chinese v2s18c_autumn_amber_afa071db:

    # aut "I swear! I have no idea what strain it is, I've never asked."
    aut "我发誓，我不知道是什么病毒，我从没问过。"

# game/v2/scene18c.rpy:1546
translate chinese v2s18c_autumn_amber_d49ed31c:

    # am "Really?!"
    am "真的吗！"

# game/v2/scene18c.rpy:1551
translate chinese v2s18c_autumn_amber_4017686a:

    # aut "Really."
    aut "真的。"

# game/v2/scene18c.rpy:1556
translate chinese v2s18c_autumn_amber_97a271e7:

    # am "Autumn, your entire world is going to open up once you start exploring different strains."
    am "秋天，你的整个世界都将打开一旦你开始探索不同的品种。"

# game/v2/scene18c.rpy:1561
translate chinese v2s18c_autumn_amber_b1c6be0b:

    # aut "You think so?"
    aut "你这么认为？"

# game/v2/scene18c.rpy:1571
translate chinese v2s18c_autumn_amber_f67352cc:

    # am "There are thousands of them, dude. And you can get blends, too."
    am "有成千上万个，伙计，你也可以买混合的。"

# game/v2/scene18c.rpy:1576
translate chinese v2s18c_autumn_amber_45af36bd:

    # aut "Blends? Like coffee?"
    aut "混合咖啡? 像咖啡？"

# game/v2/scene18c.rpy:1581
translate chinese v2s18c_autumn_amber_b32eb889:

    # am "Yes! It's exactly like coffee, you basic bitch."
    am "就像咖啡一样，你个贱人。"

# game/v2/scene18c.rpy:1586
translate chinese v2s18c_autumn_amber_c18179fa:

    # aut "How dare you."
    aut "你好大的胆子。"

# game/v2/scene18c.rpy:1591
translate chinese v2s18c_autumn_amber_cc83d727:

    # u "Should I interfere in this drug debate?"
    u "我应该插手这场毒品辩论吗？"

# game/v2/scene18c.rpy:1596
translate chinese v2s18c_autumn_amber_27e78e6c:

    # am "Maybe, ha."
    am "也许吧。"

# game/v2/scene18c.rpy:1599
translate chinese v2s18c_autumn_amber_f7a0e035:

    # am "Although obviously... I don't do that anymore. But I can at least pass on my wisdom to others."
    am "虽然很明显... ... 我不再那样做了。但是我至少可以把我的智慧传授给别人。"

# game/v2/scene18c.rpy:1604
translate chinese v2s18c_autumn_amber_20ae1b45:

    # u "True, true."
    u "没错，没错。"

# game/v2/scene18c.rpy:1609
translate chinese v2s18c_autumn_amber_d09a1635:

    # am "I'm just excited to find someone interested in this stuff."
    am "我只是很高兴能找到对这些东西感兴趣的人。"

# game/v2/scene18c.rpy:1614
translate chinese v2s18c_autumn_amber_6e8d31f6:

    # u "I never would've expected you two hanging out together... Now I'm worried that I should start a bail fund."
    u "我从没想过你们俩会在一起，现在我担心我该设立一个保释基金。"

# game/v2/scene18c.rpy:1619
translate chinese v2s18c_autumn_amber_b0bdaa8c:

    # aut "Don't worry about that. I can get us out of anything... usually."
    aut "别担心，我能帮我们摆脱任何困境... 通常情况下。"

# game/v2/scene18c.rpy:1624
translate chinese v2s18c_autumn_amber_f4d59416:

    # am "Yes! That's my girl."
    am "太好了，这才是我的好姑娘。"

# game/v2/scene18c.rpy:1629
translate chinese v2s18c_autumn_amber_e1c2658c:

    # u "Oh boy..."
    u "天啊。"

# game/v2/scene18c.rpy:1634
translate chinese v2s18c_autumn_amber_e363ec99:

    # am "I'll teach you a thing or two about weed, and you can show me your big brain power."
    am "我会教你一些关于大麻的知识然后你就可以向我展示你的聪明才智了。"

# game/v2/scene18c.rpy:1639
translate chinese v2s18c_autumn_amber_8209324d:

    # aut "Okay, ha. Fair."
    aut "好吧，公平。"

# game/v2/scene18c.rpy:1644
translate chinese v2s18c_autumn_amber_9a3f8645:

    # am "I'm gonna run to the bathroom, be right back."
    am "我去趟洗手间，马上回来。"

# game/v2/scene18c.rpy:1649
translate chinese v2s18c_autumn_amber_068302d3:

    # u "I'll keep your spot warm!"
    u "我会帮你保暖的！"

# game/v2/scene18c.rpy:1654
translate chinese v2s18c_autumn_amber_ca022a4f:

    # am "You better be gone when I return!"
    am "我回来的时候你最好已经走了！"

# game/v2/scene18c.rpy:1659
translate chinese v2s18c_autumn_amber_b86ecb34:

    # aut "She's something."
    aut "她很了不起。"

# game/v2/scene18c.rpy:1664
translate chinese v2s18c_autumn_amber_061a24ff:

    # u "Yes. Yes, she is."
    u "是的，是的，她是。"

# game/v2/scene18c.rpy:1669
translate chinese v2s18c_autumn_amber_84be8bc1:

    # aut "So, what have you been up to all night?"
    aut "你一晚上都在忙什么？"

# game/v2/scene18c.rpy:1672
translate chinese v2s18c_autumn_amber_462a5c1a:

    # aut "Any luck with your little tasks?"
    aut "你的小任务有进展吗？"

# game/v2/scene18c.rpy:1682
translate chinese v2s18c_autumn_amber_f4cd1aef:

    # u "Well, you might have already heard, but I have this list of challenges that I've been working on."
    u "你可能已经听说了，但是我有一份挑战清单，我一直在努力。"

# game/v2/scene18c.rpy:1687
translate chinese v2s18c_autumn_amber_a9bfd4db:

    # aut "Yeah? I think I might have overheard something about that."
    aut "是吗，我好像无意中听到了什么。"

# game/v2/scene18c.rpy:1692
translate chinese v2s18c_autumn_amber_fdbce207:

    # u "Oh..."
    u "哦..。"

# game/v2/scene18c.rpy:1694
translate chinese v2s18c_autumn_amber_73b0fb09:

    # u "Well, I do have an easy one left, if you'd like to help me out?"
    u "好吧，我还有一个简单的，如果你愿意帮我的话？"

# game/v2/scene18c.rpy:1699
translate chinese v2s18c_autumn_amber_f8810dcb:

    # u "(Eh, I don't want to get Autumn involved in this...)"
    u "(呃，我不想把奥特姆牵扯进来...)"

# game/v2/scene18c.rpy:1701
translate chinese v2s18c_autumn_amber_a3e1eb48:

    # u "Honestly? I've just been making my rounds and trying to stay caught up with everyone."
    u "说实话，我一直在四处巡视想跟上大家的进度。"

# game/v2/scene18c.rpy:1706
translate chinese v2s18c_autumn_amber_68853ccc:

    # aut "Yeah, I get that. Parties aren't exactly my thing either."
    aut "我明白，派对也不是我的菜。"

# game/v2/scene18c.rpy:1711
translate chinese v2s18c_autumn_amber_fcff0074:

    # u "I don't mind them, there's always a lot going on though."
    u "我不介意他们，虽然总是有很多事情发生。"

# game/v2/scene18c.rpy:1716
translate chinese v2s18c_autumn_amber_38a10f60:

    # aut "Speaking of a lot... I have a shit ton of homework."
    aut "说到很多... 我有一大堆作业。"

# game/v2/scene18c.rpy:1721
translate chinese v2s18c_autumn_amber_c5446f18:

    # u "Really?"
    u "真的吗？"

# game/v2/scene18c.rpy:1726
translate chinese v2s18c_autumn_amber_ac35c0e5:

    # aut "Look, I know it's nerdy to leave a party early for-"
    aut "我知道这么早离开派对有点书呆子气"

# game/v2/scene18c.rpy:1731
translate chinese v2s18c_autumn_amber_569c471b:

    # u "You are a nerd."
    u "你是个书呆子。"

# game/v2/scene18c.rpy:1736
translate chinese v2s18c_autumn_amber_e06fb34e:

    # aut "Okay, fine! I'm a nerd!"
    aut "好吧，我是个书呆子！"

# game/v2/scene18c.rpy:1741
translate chinese v2s18c_autumn_amber_09e9245a:

    # u "You wanna shout a little louder?"
    u "你能大点声吗？"

# game/v2/scene18c.rpy:1746
translate chinese v2s18c_autumn_amber_e1bb96e9:

    # aut "Haha, anyway... I should go get some work done before bed."
    aut "哈哈，不管怎样... 我应该在睡觉前把工作做完。"

# game/v2/scene18c.rpy:1751
translate chinese v2s18c_autumn_amber_8f3f16b2:

    # u "Okay, okay. It was fun seeing you again, and great work on the decorations."
    u "好吧，好吧，很高兴再次见到你，装饰也做得很好。"

# game/v2/scene18c.rpy:1756
translate chinese v2s18c_autumn_amber_bdb875f9:

    # aut "Oh, thank you! I'm glad everyone had fun tonight. I'll see you soon."
    aut "哦，谢谢! 我很高兴大家今晚都玩得很开心。一会儿见。"

# game/v2/scene18c.rpy:1761
translate chinese v2s18c_autumn_amber_adc3d887:

    # u "Later. Good luck."
    u "回见，祝你好运。"

# game/v2/scene18c.rpy:1777
translate chinese v2s18c_autumn_amber_b2ee5271:

    # u "Ha, yeah. Some things are complete. Still have a few left, though."
    u "是啊，有些东西是完整的，不过还有一些。"

# game/v2/scene18c.rpy:1779
translate chinese v2s18c_autumn_amber_7b2935db:

    # u "Not really, still have a few left to complete."
    u "还没有，还有一些要完成。"

# game/v2/scene18c.rpy:1784
translate chinese v2s18c_autumn_amber_e38c693d:

    # aut "Hmm, like what?"
    aut "比如呢？"

# game/v2/scene18c.rpy:1793
translate chinese v2s18c_autumn_amber_5888cd9b:

    # u "(I can't ask Autumn to make out with me! What am I thinking?)"
    u "(我不能要秋天和我亲热! 我在想什么?)"

# game/v2/scene18c.rpy:1795
translate chinese v2s18c_autumn_amber_58573d33:

    # u "Actually, it's okay. Never mind."
    u "事实上，没关系，算了。"

# game/v2/scene18c.rpy:1800
translate chinese v2s18c_autumn_amber_724bb916:

    # u "It's a really dumb list."
    u "这名单太蠢了。"

# game/v2/scene18c.rpy:1805
translate chinese v2s18c_autumn_amber_d9353e8a:

    # aut "Yeah, well... you did get it from Imre."
    aut "是啊，你确实是从艾姆雷那里拿到的。"

# game/v2/scene18c.rpy:1810
translate chinese v2s18c_autumn_amber_f93a8ae8:

    # u "Very true."
    u "没错。"

# game/v2/scene18c.rpy:1812
translate chinese v2s18c_autumn_amber_5c701db7:

    # u "Anyway, I'm gonna go catch everyone else before the party ends."
    u "不管怎样，我要在派对结束前去找其他人。"

# game/v2/scene18c.rpy:1817
translate chinese v2s18c_autumn_amber_339fa3bc:

    # aut "Okay, sounds good. Later. I gotta head home to study anyways."
    aut "好的，听起来不错，回见，反正我也要回家学习了。"

# game/v2/scene18c.rpy:1827
translate chinese v2s18c_autumn_amber_e1704450:

    # u "Well, do you wanna make out?"
    u "你想亲热一下吗？"

# game/v2/scene18c.rpy:1833
translate chinese v2s18c_autumn_amber_98b23d0f:

    # aut "[name]! You're dating my sister. What are you-"
    aut "[name]! 你约会我的妹妹。你是什么-"

# game/v2/scene18c.rpy:1838
translate chinese v2s18c_autumn_amber_f5b43cd7:

    # u "Yeah, I know, I'm sorry... I just-"
    u "我知道，对不起，我只是.."

# game/v2/scene18c.rpy:1843
translate chinese v2s18c_autumn_amber_b71d5a88:

    # aut "You just nothing. You're lucky I won't tell Lauren that you even asked me."
    aut "你什么都不是，你很幸运我不会告诉劳伦你问过我。"

# game/v2/scene18c.rpy:1848
translate chinese v2s18c_autumn_amber_76ec5f31:

    # u "I'm sorry, Autumn. I thought-"
    u "对不起，奥顿，我以为.."

# game/v2/scene18c.rpy:1853
translate chinese v2s18c_autumn_amber_35707dd3:

    # aut "This never happened, [name]. Go away."
    aut "这事从没发生过,[name]。走开。"

# game/v2/scene18c.rpy:1858
translate chinese v2s18c_autumn_amber_22f8f829:

    # u "Yeah. Okay..."
    u "好吧。"

# game/v2/scene18c.rpy:1863
translate chinese v2s18c_autumn_amber_3bf3cdce:

    # u "(Well, fuck...)"
    u "(好吧，他妈的...)"

# game/v2/scene18c.rpy:1868
translate chinese v2s18c_autumn_amber_ba34248b:

    # aut "I'm just gonna leave and go check on Amber."
    aut "我要走了，去看看安珀。"

# game/v2/scene18c.rpy:1874
translate chinese v2s18c_autumn_amber_7a669a23:

    # aut "Ha, I uh..."
    aut "我..。"

# game/v2/scene18c.rpy:1876
translate chinese v2s18c_autumn_amber_82375792:

    # aut "There's too much drama, I think... between you and Lauren."
    aut "我觉得你和劳伦之间有太多戏剧性的事情了。"

# game/v2/scene18c.rpy:1878
translate chinese v2s18c_autumn_amber_fc970a72:

    # aut "It's just hard to-"
    aut "只是很难.."

# game/v2/scene18c.rpy:1883
translate chinese v2s18c_autumn_amber_06c69842:

    # u "I understand. No worries."
    u "我明白，别担心。"

# game/v2/scene18c.rpy:1888
translate chinese v2s18c_autumn_amber_bc4d3b8f:

    # aut "Okay, thanks."
    aut "好的，谢谢。"

# game/v2/scene18c.rpy:1890
translate chinese v2s18c_autumn_amber_0dd7fc70:

    # aut "I'm gonna go force Amber out of the bathroom! I have to pee really bad..."
    aut "我要去把安珀赶出厕所，我真的很想尿尿。"

# game/v2/scene18c.rpy:1895
translate chinese v2s18c_autumn_amber_b645db82:

    # u "Ha, okay. See you around."
    u "好吧，回头见。"

# game/v2/scene18c.rpy:1900
translate chinese v2s18c_autumn_amber_8a1dedb2:

    # aut "Later!"
    aut "回见！"

# game/v2/scene18c.rpy:1905
translate chinese v2s18c_autumn_amber_7b978365:

    # u "(Well, that could've gone worse...)"
    u "(好吧，那可能会更糟...)"

# game/v2/scene18c.rpy:1915
translate chinese v2s18c_autumnsg_4ea05f33:

    # aut "Ha! What?"
    aut "哈! 什么？"

# game/v2/scene18c.rpy:1920
translate chinese v2s18c_autumnsg_2ab13b3b:

    # u "I have to make out with someone."
    u "我得找个人亲热一下。"

# game/v2/scene18c.rpy:1925
translate chinese v2s18c_autumnsg_41634883:

    # aut "Oh... sure. That's easy."
    aut "当然，很简单。"

# game/v2/scene18c.rpy:1930
translate chinese v2s18c_autumnsg_e7dacb23:

    # u "Yeah!"
    u "耶！"

# game/v2/scene18c.rpy:1932
translate chinese v2s18c_autumnsg_a11d55ea:

    # u "Wait, really?"
    u "等等，真的吗？"

# game/v2/scene18c.rpy:1942
translate chinese v2s18c_autumnsg_8064c10c:

    # aut "Hmm... I don't-"
    aut "嗯... 我不.."

# game/v2/scene18c.rpy:1947
translate chinese v2s18c_autumnsg_3d26c476:

    # u "It's cool, no worries."
    u "没关系，别担心。"

# game/v2/scene18c.rpy:1952
translate chinese v2s18c_autumnsg_39e80ff4:

    # aut "I'm just not feeling it... I'm sorry, [name]."
    aut "我只是没感觉... 对不起,[name]."

# game/v2/scene18c.rpy:1957
translate chinese v2s18c_autumnsg_aa1b7352:

    # u "Really, it's okay."
    u "真的，没关系。"

# game/v2/scene18c.rpy:1962
translate chinese v2s18c_autumnsg_b6efd30b:

    # aut "Thanks."
    aut "谢谢。"

# game/v2/scene18c.rpy:1967
translate chinese v2s18c_autumnsg_0dd7fc70:

    # aut "I'm gonna go force Amber out of the bathroom! I have to pee really bad..."
    aut "我要去把安珀赶出厕所，我真的很想尿尿。"

# game/v2/scene18c.rpy:1972
translate chinese v2s18c_autumnsg_921422e9:

    # u "Gotcha. See you around."
    u "知道了，回头见。"

# game/v2/scene18c.rpy:1977
translate chinese v2s18c_autumnsg_8a1dedb2:

    # aut "Later!"
    aut "回见！"

# game/v2/scene18c.rpy:1982
translate chinese v2s18c_autumnsg_7b978365:

    # u "(Well, that could've gone worse...)"
    u "(好吧，那可能会更糟...)"

# game/v2/scene18c.rpy:2005
translate chinese v2s18c_autumn_kiss_7fc93f43:

    # aut "Let's make this quick. Under the stairs here, while nobody is looking."
    aut "我们速战速决，趁没人注意，躲到楼梯下面去。"

# game/v2/scene18c.rpy:2010
translate chinese v2s18c_autumn_kiss_2ca9fab5:

    # u "Perfect."
    u "好极了。"

# game/v2/scene18c.rpy:2015
translate chinese v2s18c_autumn_kiss_b959c4d7:

    # aut "And no tongue, [name]."
    aut "没有舌头,[name]."

# game/v2/scene18c.rpy:2020
translate chinese v2s18c_autumn_kiss_83b73476:

    # u "Ha! Okay, no tongues."
    u "哈! 好吧，不要舌头。"

# game/v2/scene18c.rpy:2049
translate chinese v2s18c_autumn_kiss_bbab2974:

    # aut "There. Good?"
    aut "好了，可以了吗？"

# game/v2/scene18c.rpy:2054
translate chinese v2s18c_autumn_kiss_bf7776d2:

    # u "Mhmm... yeah, all good."
    u "嗯... 是的，都很好。"

# game/v2/scene18c.rpy:2059
translate chinese v2s18c_autumn_kiss_4a968c12:

    # aut "Then we're set."
    aut "那就这么定了。"

# game/v2/scene18c.rpy:2064
translate chinese v2s18c_autumn_kiss_378a8243:

    # u "Thanks, Autumn."
    u "谢谢，奥顿。"

# game/v2/scene18c.rpy:2069
translate chinese v2s18c_autumn_kiss_99250dd0:

    # aut "Mhmm... now, go away."
    aut "嗯... 现在，走开。"

# game/v2/scene18c.rpy:2077
translate chinese v2s18c_autumn_kiss_3c8517fb:

    # u "Sure thing."
    u "没问题。"

# game/v2/scene18c.rpy:2093
translate chinese v2s18c_chris_penelope_bdfb9c2d:

    # ch "Yeah, but... you know what it's like."
    ch "是的，但是... 你知道那是什么感觉。"

# game/v2/scene18c.rpy:2095
translate chinese v2s18c_chris_penelope_1698e3ba:

    # ch "You've been single for how long now?"
    ch "你单身多久了？"

# game/v2/scene18c.rpy:2101
translate chinese v2s18c_chris_penelope_1ccb363a:

    # pe "Oh, I'm... Well..."
    pe "我..。"

# game/v2/scene18c.rpy:2103
translate chinese v2s18c_chris_penelope_1503fb58:

    # pe "A while, I guess..."
    pe "有一段时间了，我想..。"

# game/v2/scene18c.rpy:2109
translate chinese v2s18c_chris_penelope_0b0b4577:

    # ch "You have a boyfriend? Sorry, I didn't know."
    ch "你有男朋友了? 对不起，我不知道。"

# game/v2/scene18c.rpy:2111
translate chinese v2s18c_chris_penelope_29649e73:

    # ch "So, how do you deal with it?"
    ch "那你是怎么处理的？"

# game/v2/scene18c.rpy:2117
translate chinese v2s18c_chris_penelope_36f3fa11:

    # pe "Well, I don't... have a boyfriend?"
    pe "我没有男朋友吗？"

# game/v2/scene18c.rpy:2119
translate chinese v2s18c_chris_penelope_fb456fd7:

    # ch "..."
    ch "..."

# game/v2/scene18c.rpy:2121
translate chinese v2s18c_chris_penelope_8fd4e488:

    # pe "With what?"
    pe "用什么？"

# game/v2/scene18c.rpy:2127
translate chinese v2s18c_chris_penelope_1e39e915:

    # ch "So, you're single?"
    ch "你单身吗？"

# game/v2/scene18c.rpy:2129
translate chinese v2s18c_chris_penelope_b365040f:

    # ch "Being single."
    ch "单身。"

# game/v2/scene18c.rpy:2135
translate chinese v2s18c_chris_penelope_ef273def:

    # u "(Jesus, dude. Back off.)"
    u "天啊，老兄，退后"

# game/v2/scene18c.rpy:2137
translate chinese v2s18c_chris_penelope_4d823086:

    # u "(Fuck's sake, Chris...)"
    u "(妈的，克里斯...)"

# game/v2/scene18c.rpy:2143
translate chinese v2s18c_chris_penelope_ac08abd7:

    # pe "No, I don't have a boyfriend... but I'm not single. I'm seeing someone? I think?"
    pe "不，我没有男朋友，但我不是单身，我在和别人约会? 我想？"

# game/v2/scene18c.rpy:2145
translate chinese v2s18c_chris_penelope_5313fb61:

    # pe "Does that count?"
    pe "这算吗？"

# game/v2/scene18c.rpy:2150
translate chinese v2s18c_chris_penelope_9bc6fbc9:

    # ch "Sure. I guess?"
    ch "当然，我想是吧？"

# game/v2/scene18c.rpy:2155
translate chinese v2s18c_chris_penelope_340a8f8b:

    # pe "So... Yeah."
    pe "所以... 是的。"

# game/v2/scene18c.rpy:2160
translate chinese v2s18c_chris_penelope_9aba9c42:

    # u "(This is painful to listen to.)"
    u "(听起来很痛苦。)"

# game/v2/scene18c.rpy:2163
translate chinese v2s18c_chris_penelope_1bedd63e:

    # pe "I don't understand..."
    pe "我不明白..。"

# game/v2/scene18c.rpy:2168
translate chinese v2s18c_chris_penelope_0c407578:

    # ch "Ha, sorry. I'm not really thinking straight."
    ch "哈，抱歉，我没想清楚。"

# game/v2/scene18c.rpy:2170
translate chinese v2s18c_chris_penelope_359a3da6:

    # ch "I'm such a lightweight these days!"
    ch "我最近喝得太少了！"

# game/v2/scene18c.rpy:2175
translate chinese v2s18c_chris_penelope_da5e1fb8:

    # pe "Oh, ha... okay."
    pe "好吧。"

# game/v2/scene18c.rpy:2184
translate chinese v2s18c_chris_penelope_7016a499:

    # u "(Sorry, Penelope. I don't want anything to do with sad boy Chris, I'm outta here!)"
    u "(对不起，佩内洛普，我不想和悲伤的男孩克里斯有任何关系，我要走了!)"

# game/v2/scene18c.rpy:2193
translate chinese v2s18c_chris_penelope_4caec112:

    # u "Hey, Chris."
    u "嘿，克里斯。"

# game/v2/scene18c.rpy:2198
translate chinese v2s18c_chris_penelope_4a80488d:

    # ch "Oh, hey [name]."
    ch "哦，嘿[name]."

# game/v2/scene18c.rpy:2203
translate chinese v2s18c_chris_penelope_3beed5a2:

    # u "I think Imre was looking for you. It sounded important."
    u "我想伊姆雷在找你，听起来很重要。"

# game/v2/scene18c.rpy:2205
translate chinese v2s18c_chris_penelope_dac7b969:

    # u "Something about Wolves and fighting."
    u "关于狼和战斗的事。"

# game/v2/scene18c.rpy:2210
translate chinese v2s18c_chris_penelope_5893d92a:

    # ch "My whole life revolves around Wolves and fighting... well, now that Nora's gone..."
    ch "我的整个人生都围绕着狼群和战斗，现在诺拉走了。"

# game/v2/scene18c.rpy:2215
translate chinese v2s18c_chris_penelope_176d10c0:

    # ch "Anyway, thanks man. See you guys."
    ch "不管怎样，谢谢你，再见。"

# game/v2/scene18c.rpy:2225
translate chinese v2s18c_chris_penelope_6f0fda28:

    # pe "My hero! Thank you, [name]."
    pe "我的英雄，谢谢,[name]."

# game/v2/scene18c.rpy:2230
translate chinese v2s18c_chris_penelope_5c4e5a55:

    # u "You're welcome. I could see the pain in your eyes."
    u "不客气，我能看出你眼中的痛苦。"

# game/v2/scene18c.rpy:2235
translate chinese v2s18c_chris_penelope_6e9bea9e:

    # pe "I've been stuck in that conversation for twenty minutes now..."
    pe "我已经被这个话题困住20分钟了。"

# game/v2/scene18c.rpy:2237
translate chinese v2s18c_chris_penelope_435e7dab:

    # pe "Chris is a nice guy, but damn! He can be boring as hell sometimes..."
    pe "克里斯是个好人，但该死的! 他有时候真的很无聊..。"

# game/v2/scene18c.rpy:2242
translate chinese v2s18c_chris_penelope_682fbf25:

    # u "Yeah. He's going through some stuff right now."
    u "是的，他现在正在经历一些事情。"

# game/v2/scene18c.rpy:2247
translate chinese v2s18c_chris_penelope_bdcb161e:

    # pe "*Sighs* True."
    pe "* 叹息 * 真的。"

# game/v2/scene18c.rpy:2253
translate chinese v2s18c_chris_penelope_002088a4:

    # pe "So, I heard about a list you've been working on..."
    pe "我听说你最近在做一个名单。"

# game/v2/scene18c.rpy:2258
translate chinese v2s18c_chris_penelope_fab3fb3f:

    # u "Oh? From whom?"
    u "是吗，谁说的？"

# game/v2/scene18c.rpy:2263
translate chinese v2s18c_chris_penelope_f7793ca5:

    # pe "Does it matter? Let me see."
    pe "有关系吗? 让我看看。"

# game/v2/scene18c.rpy:2273
translate chinese v2s18c_chris_penelope_d934b260:

    # pe "Oooooh, wow. Okay..."
    pe "好吧。"

# game/v2/scene18c.rpy:2283
translate chinese v2s18c_chris_penelope_037deaf9:

    # u "Yeah, Imre gave it to me."
    u "是的，伊姆雷给我的。"

# game/v2/scene18c.rpy:2288
translate chinese v2s18c_chris_penelope_e782d475:

    # pe "That makes complete sense."
    pe "这完全说得通。"

# game/v2/scene18c.rpy:2294
translate chinese v2s18c_chris_penelope_57f7673b:

    # u "So, about that list I showed you earlier..."
    u "关于我之前给你看的名单。"

# game/v2/scene18c.rpy:2299
translate chinese v2s18c_chris_penelope_65cf10b1:

    # pe "Oh, yeah... that thing."
    pe "Oh, yeah... that thing."

# game/v2/scene18c.rpy:2306
translate chinese v2s18c_chris_penelope_6993575b:

    # u "How would you feel about helping me check off one of these?"
    u "帮我检查一下这个怎么样？"

# game/v2/scene18c.rpy:2311
translate chinese v2s18c_chris_penelope_a76f51ff:

    # u "It's so stupid, right?"
    u "太蠢了，对吧？"

# game/v2/scene18c.rpy:2316
translate chinese v2s18c_chris_penelope_d8dd28fe:

    # pe "Yeah. But it might also get some people really lucky. If you know what I mean."
    pe "是的。但是它也可能让一些人真的很幸运。如果你知道我的意思。"

# game/v2/scene18c.rpy:2321
translate chinese v2s18c_chris_penelope_c100709e:

    # u "I think I do."
    u "我想我知道。"

# game/v2/scene18c.rpy:2330
translate chinese v2s18c_askhelp_f2081299:

    # pe "Um, I don't think we should do anything while we're here."
    pe "我觉得我们在这里什么都不应该做。"

# game/v2/scene18c.rpy:2335
translate chinese v2s18c_askhelp_923343fe:

    # u "Oh, okay, sure."
    u "好的，当然。"

# game/v2/scene18c.rpy:2340
translate chinese v2s18c_askhelp_3e1f5d40:

    # pe "Sorry, I just don't feel comfortable."
    pe "对不起，我只是觉得不舒服。"

# game/v2/scene18c.rpy:2350
translate chinese v2s18c_penelopesg_708d7db0:

    # pe "So... um... what exactly do you need help with, anyway?"
    pe "那么... 你到底需要我帮什么忙？"

# game/v2/scene18c.rpy:2355
translate chinese v2s18c_penelopesg_79765266:

    # u "Follow me."
    u "跟我来。"

# game/v2/scene18c.rpy:2368
translate chinese v2s18c_penelopesg_348a3b9f:

    # u "I want to do something for you."
    u "我想为你做点事。"

# game/v2/scene18c.rpy:2373
translate chinese v2s18c_penelopesg_06ca135a:

    # pe "And what would that be?"
    pe "那是什么？"

# game/v2/scene18c.rpy:2378
translate chinese v2s18c_penelopesg_05181888:

    # u "Get comfortable."
    u "舒服点。"

# game/v2/scene18c.rpy:2396
translate chinese v2s18c_penelopesg_7f2da20c:

    # pe "Oh... oh my..."
    pe "哦... 哦，我的..。"

# game/v2/scene18c.rpy:2398
translate chinese v2s18c_penelopesg_d8765019:

    # pe "I see where this is going."
    pe "我知道怎么回事了。"

# game/v2/scene18c.rpy:2408
translate chinese v2s18c_penelopesg_06752751:

    # pe "Mmm..."
    pe "嗯..。"

# game/v2/scene18c.rpy:2410
translate chinese v2s18c_penelopesg_1c8f7a54:

    # pe "Oh... [name]..."
    pe "哦..。[name]..."

# game/v2/scene18c.rpy:2412
translate chinese v2s18c_penelopesg_46e686ff:

    # pe "I couldn't have imagined..."
    pe "我无法想象..。"

# game/v2/scene18c.rpy:2414
translate chinese v2s18c_penelopesg_a437d78a:

    # u "Do you like it when I taste you?"
    u "你喜欢我尝你的味道吗？"

# game/v2/scene18c.rpy:2416
translate chinese v2s18c_penelopesg_81bb48f1:

    # pe "I... I really do, [name]..."
    pe "我... 我真的很想,[name]..."

# game/v2/scene18c.rpy:2418
translate chinese v2s18c_penelopesg_eaf77367:

    # pe "You... *Gasps*"
    pe "你... * 倒抽一口凉气 * "

# game/v2/scene18c.rpy:2425
translate chinese v2s18c_penelopesg_dd030910:

    # pe "You really know what you're doing! Down there, I mean..."
    pe "你真的知道你在做什么! 在那里，我的意思是..。"

# game/v2/scene18c.rpy:2427
translate chinese v2s18c_penelopesg_66184048:

    # u "*Muffled* Mhmm..."
    u "* 闷闷不乐 * 。"

# game/v2/scene18c.rpy:2429
translate chinese v2s18c_penelopesg_86b8ea7c:

    # pe "Yes, yes! *Moans* Just like that..."
    pe "是的，是的! * 呻吟 * 就像这样..。"

# game/v2/scene18c.rpy:2431
translate chinese v2s18c_penelopesg_b2a3fa99:

    # pe "Oh, I can't believe we're doing this."
    pe "真不敢相信我们要这么做。"

# game/v2/scene18c.rpy:2433
translate chinese v2s18c_penelopesg_4fddcc39:

    # u "(I can't believe how good you taste...)"
    u "(我不敢相信你的味道这么好...)"

# game/v2/scene18c.rpy:2435
translate chinese v2s18c_penelopesg_fbec25ac:

    # pe "I'm going to cum! [name], I-"
    pe "我要射了！[name]我."

# game/v2/scene18c.rpy:2443
translate chinese v2s18c_penelopesg_99e9eacd:

    # pe "I'm..."
    pe "我..。"

# game/v2/scene18c.rpy:2448
translate chinese v2s18c_penelopesg_26aaac98:

    # pe "*Gasps*"
    pe "* 喘息 *"

# game/v2/scene18c.rpy:2453
translate chinese v2s18c_penelopesg_a0de8459:

    # pe "Ha..."
    pe "哈..。"

# game/v2/scene18c.rpy:2455
translate chinese v2s18c_penelopesg_3aed3db4:

    # pe "You made me cum. *Panting*"
    pe "你让我高潮了 * 气喘吁吁 * "

# game/v2/scene18c.rpy:2462
translate chinese v2s18c_penelopesg_3bab75dc:

    # u "I could tell."
    u "我看得出来。"

# game/v2/scene18c.rpy:2488
translate chinese v2s18c_penelopesg_784ea6a5:

    # pe "That was incredible..."
    pe "太不可思议了。"

# game/v2/scene18c.rpy:2493
translate chinese v2s18c_penelopesg_1a5c75e1:

    # u "I'm glad you enjoyed it. Thanks for your help."
    u "很高兴你喜欢，谢谢你的帮助。"

# game/v2/scene18c.rpy:2498
translate chinese v2s18c_penelopesg_05eb33c8:

    # pe "*Sighs* You're so welcome..."
    pe "不客气。"

# game/v2/scene18c.rpy:2500
translate chinese v2s18c_penelopesg_62aa2066:

    # pe "I could lay here forever..."
    pe "我可以永远躺在这里。"

# game/v2/scene18c.rpy:2505
translate chinese v2s18c_penelopesg_8c7b7c16:

    # u "People might start to wonder where we are, but, same."
    u "人们可能会开始怀疑我们在哪里，但是，一样的。"

# game/v2/scene18c.rpy:2510
translate chinese v2s18c_penelopesg_69ff3af7:

    # pe "You're right."
    pe "你说得对。"

# game/v2/scene18c.rpy:2526
translate chinese v2s18c_nsfwSkipLabelP_d2a3f50d:

    # pe "Ready?"
    pe "准备好了吗？"

# game/v2/scene18c.rpy:2534
translate chinese v2s18c_nsfwSkipLabelP_a808a3a1:

    # u "After you."
    u "你先请。"

# game/v2/scene18c.rpy:2552
translate chinese v2s18c_nsfwSkipLabelP_722d8eae:

    # pe "I think it would be weird if we did anything on this list together."
    pe "我觉得如果我们一起做这个单子上的事情会很奇怪。"

# game/v2/scene18c.rpy:2554
translate chinese v2s18c_nsfwSkipLabelP_e74e1627:

    # pe "Since we're just friends. You know?"
    pe "因为我们只是朋友，你知道吗？"

# game/v2/scene18c.rpy:2559
translate chinese v2s18c_nsfwSkipLabelP_daac62bb:

    # u "Yeah, I get that."
    u "是的，我明白。"

# game/v2/scene18c.rpy:2564
translate chinese v2s18c_nsfwSkipLabelP_a50382cb:

    # pe "Sorry."
    pe "对不起。"

# game/v2/scene18c.rpy:2573
translate chinese v2s18c_nsfwSkipLabelP_f2081299:

    # pe "Um, I don't think we should do anything while we're here."
    pe "我觉得我们在这里什么都不应该做。"

# game/v2/scene18c.rpy:2578
translate chinese v2s18c_nsfwSkipLabelP_923343fe:

    # u "Oh, okay, sure."
    u "好的，当然。"

# game/v2/scene18c.rpy:2583
translate chinese v2s18c_nsfwSkipLabelP_3e1f5d40:

    # pe "Sorry, I just don't feel comfortable."
    pe "对不起，我只是觉得不舒服。"

# game/v2/scene18c.rpy:2588
translate chinese v2s18c_nsfwSkipLabelP_dfa1ec00:

    # u "Trust me, don't worry about it."
    u "相信我，别担心。"

# game/v2/scene18c.rpy:2593
translate chinese v2s18c_nsfwSkipLabelP_5d446ccc:

    # pe "Okay. Thanks."
    pe "好的，谢谢。"

# game/v2/scene18c.rpy:2599
translate chinese v2s18c_backtoparty_73ab2e1e:

    # u "Anyway, time to get back to the party. Ready?"
    u "不管怎样，该回派对了，准备好了吗？"

# game/v2/scene18c.rpy:2604
translate chinese v2s18c_backtoparty_6d702833:

    # pe "Yes! So ready."
    pe "太好了，准备好了。"

translate chinese strings:

    # game/v2/scene18c.rpy:61
    old "You can't shop for yourself"
    new "你不能为自己购物"

    # game/v2/scene18c.rpy:614
    old "Ask for help"
    new "寻求帮助"

    # game/v2/scene18c.rpy:614
    old "Don't mention it"
    new "别客气"

    # game/v2/scene18c.rpy:865
    old "Keep her quiet"
    new "让她安静"

    # game/v2/scene18c.rpy:865
    old "Let her be loud"
    new "让她大声点"

    # game/v2/scene18c.rpy:1109
    old "Steal the panties"
    new "偷内裤"

    # game/v2/scene18c.rpy:1109
    old "Don't steal the panties"
    new "别偷内裤"

    # game/v2/scene18c.rpy:1241
    old "Take the condom"
    new "带上避孕套"

    # game/v2/scene18c.rpy:1352
    old "This balcony is better"
    new "这个阳台更好"

    # game/v2/scene18c.rpy:1352
    old "Bigger the better"
    new "越大越好"

    # game/v2/scene18c.rpy:1789
    old "Never mind"
    new "算了"

    # game/v2/scene18c.rpy:1789
    old "Let's make out"
    new "我们亲热吧"

    # game/v2/scene18c.rpy:2180
    old "Don't interrupt"
    new "别打断我"

    # game/v2/scene18c.rpy:2180
    old "Save Penelope"
    new "拯救佩内洛普"

    # game/v2/scene18c.rpy:2304
    old "Ask for her help"
    new "找她帮忙"

    # game/v2/scene18c.rpy:2304
    old "Tell her the list is stupid"
    new "告诉她名单很蠢"

